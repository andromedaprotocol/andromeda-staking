        ...
        options: {
          ...
          customBodyRender: (item) => {
            let value = this.props.delegations.find(
              (val) =>
                (val.delegation && val.delegation.validator_address) ===
        ...
        options: {
          ...
          customBodyRender: (validatorAddress) =>
            this.props.delegations.find(
              (item) =>
                (item.delegation && item.delegation.validator_address) ===
                validatorAddress
            ) ? (
    ...
    const tableData =
      dataToMap && dataToMap.length
        ? dataToMap.map((item) => [
            ...
            item,
            parseFloat(
              (Number(item.tokens) / 10 ** config.COIN_DECIMALS).toFixed(1)
            ),
            item.commission &&
            item.commission.commission_rates &&
            item.commission.commission_rates.rate
              ? parseFloat(
                  (Number(item.commission.commission_rates.rate) * 100).toFixed(
                    2
                  )
                )
              : null,
